...
namespace Americasa.Demo.CustomActivities.Stores
{
    public interface ITestTriggerStore : ITestStore<Trigger>
    {
    }
    public interface ITestStore<T> where T : IEntity
    {
    ...
    public abstract class TestElsaContextEntityFrameworkStore<T> : TestEntityFrameworkStore<T, ElsaContext> where T : class, IEntity
    {
        protected TestElsaContextEntityFrameworkStore(IContextFactory<ElsaContext> dbContextFactory, IMapper mapper) : base(dbContextFactory, mapper)
        {
        }
    }

    public class TestEntityFrameworkTriggerStore : TestElsaContextEntityFrameworkStore<Trigger>, ITestTriggerStore
    {
        public TestEntityFrameworkTriggerStore(IElsaContextFactory dbContextFactory, IMapper mapper) : base(dbContextFactory, mapper)
        {
        }
    ...
    public abstract class TestEntityFrameworkStore<
#if NET7_0_OR_GREATER
...
#endif
    T, TContext> : ITestStore<T> where T : class, IEntity where TContext : DbContext
    {
        private readonly IMapper _mapper;
        private readonly SemaphoreSlim _semaphore = new(1);

        protected TestEntityFrameworkStore(IContextFactory<TContext> dbContextFactory, IMapper mapper)
        {
            _mapper = mapper;
